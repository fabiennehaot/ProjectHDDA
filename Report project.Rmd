...
We notice that the PCs do not clearly separate the accepted and rejected cases. However, we do see that the rejected cases are more present in the bottom left corner and the accepted cases in the top right corner. Figure `r fig_nr-1`B confirms this observation by illustrating that the density curve of the rejected cases (blue) is situated where the values for PC1 and PC2 are low in contrary to the density curve of the accepted cases (red), which is situated where PC1 and PC2 are higher.
...
For both PC1 and PC2 around 200 genes are important (Figure 4B), which can provide more or less the same result as the PCA with all the genes (Figure `r fig_nr-1`A). This subset of genes are the drivers of the variablity in X. However, because PCA does not separate the rejected and accepted cases well, these genes won't provide much information on whether or not the transpantation will be successful or not.
