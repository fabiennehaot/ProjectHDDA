...
hist_zval <- ggplot(data=t_stats) + 
  ...
  geom_line(mapping = aes(x = z_val,y = dnorm(z_val)),
            color = 'blue')+
  labs(title = "Figure 9= Histogram of z-values", x = "z-value", y = "density") +
...
hist_pval <- ggplot(data = t_stats) +
  ...
  labs(title = 'Figure 10: Histogram of p-values', x = "p-value") +
  theme_bw()
...
hist_fdr <- ggplot(t_stats) + 
  ...
  geom_line(mapping = aes(x=z_scale, y = dnorm(z_scale)),
            color='blue') +
  labs(title = 'Figure 11: Histogram of z-values') +
  theme_bw()            
...
hist_pscale <- ggplot(data = t_stats) +
  geom_histogram(mapping = aes(x = p_scale),
                 fill = "firebrick", bins = 20, alpha = .7) +
  labs(title = 'Figure 12: Histogram of p-values') +
  theme_bw()
...
pp_adj <- t_stats %>%
  ...
  labs(title = "Figure 13: Adjusting the p-values",
       y = "adjusted p-value") +
  theme_bw()
