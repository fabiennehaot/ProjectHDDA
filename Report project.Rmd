...
grid.arrange(p_scatter, p_dens, ncol = 2, top = "Figure 2: Principal component analysis")
```

The first 2 PCs explain 19% of the variability of X and with the first 50 we obtain 63% of the variability of X. 

To get more insight in the research question, we examine whether kidney rejection is associated with the PCs. For this, we look Figure 2 A. 
We notice that the PCs do not clearly separate the accepted and rejected cases. However, we do see that the rejected cases are more present in the bottom left corner and the accepted cases in the top right corner. + interpretation figure 2B
...
plot(SPC1, SPC2,
     ...
     main = paste("B.", non_zero1, "genes for SPC1 \n and", non_zero2, "genes for SPC2")
)
...
grid.arrange(hist_zval, hist_pval, ncol = 2,
             top = "Figure 6: Multiple t-tests unadjusted")
...
grid.arrange(hist_fdr, hist_pscale, ncol=2,
             top  = "Figure 7: Multiple t-tests: scaling and FDR-adjustment")
...
#local fdr
fdr_x <- locfdr(t_stats$z_val, plot = 4, main = "A. Histogram")  #add here maintitle Figure 8 and change title other plots?
prop_h0 <- fdr_x$fp0['mlest', 'p0']
mtext("Figure 8", outer = TRUE, cex = 1.5)
...
grid.arrange(plot_lfdr, top = "Figure 9: Multiple t-tests: Local FDR")
...
grid.arrange(p1, p2, ncol = 2, top = "Figure 10: Optimal number of PCs") # plot the 2 plots next to each other
```

In a second step, we loop and repeat this for each PC. First, we loop over the 175 PCs, in steps of 10, to decide in which region we will zoom in. Here, we get `r nr_pc_opt1` as optimal number of PCs. To get a more accurate look, we zoom in between 1 and 20 PCs and repeat the same process, but with steps of 1. This gives us an optimal number of `r nr_pc_opt2` PCs.
...
mtext("Figure 12: Lasso regression", outer = TRUE, cex = 1.5)
```
...
plot(mRidge, xvar = "lambda", main = "Figure 14: Coefficients ridge regression")
...
Next, we evaluate this final model on our test set. First, the corresponding ROC curve is examined. Our model clearly performs better than random allocation of cases to the 2 groups.
...
print(roc + ggtitle("Figure 15: ROC curve"))
