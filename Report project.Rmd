...
For both PC1 and PC2 around 200 genes are important (Figure `r fig_nr-1`), which can provide more or less the same result as the PCA with all the genes (Figure `r fig_nr-2`). This subset of genes are the drivers of the variablity in X. However, because PCA does not separate the rejected and accepted cases well, these genes won't provide much information on whether or not the transplantation will be successful or not.
