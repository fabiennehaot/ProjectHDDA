...
ggplot(data = tb_summary) +
  ...
  ggtitle("Figure 1: Data exploration") +
  theme_bw() +
...
par(pch = 19, mfrow = c(1, 2), oma = c(0, 0, 2, 0))

hist(V[, 1], breaks = 50, xlab = "PC 1 loadings", main = "A. Histogram PC 1 loadings")
abline(v = c(
  quantile(V[, 1], 0.05),
  quantile(V[, 1], 0.95)
), col = "red", lwd = 2)

hist(V[, 2], breaks = 50, xlab = "PC 2 loadings", main = "B. Histogram PC 2 loadings")
abline(v = c(
  quantile(V[, 2], 0.05),
  quantile(V[, 2], 0.95)
), col = "red", lwd = 2)

mtext("Figure 3: Loadings", outer = TRUE, cex = 1.5)
```

The histograms (Figures 3.A and 3.B) confirm this finding, which gives us a reason to believe that sparse PCA is worthwhile. 
...
mtext("Figure 4: Visualization of the first two PCs", outer = TRUE, cex = 1.5)
```
...
boxplot(Zlda ~ Y, col = cols, ylab = expression("Z"[1]),
        main = "A. LDA with total set of genes")
...
boxplot(SLDA ~ Y,
        col = cols, ylab = "SLDA",
        main = sprintf("B. LDA with subset of %d genes", n_nonzero)
)

mtext("Figure 5: Linear discriminant analysis", outer = TRUE, cex = 1.5)
```
...
When we plot the two densities of the accepted and rejected kidneys with this model (Figure 11), we clearly see there is no overlap. This means, the model is not acceptable enough. 

### Lasso regression

We are now going to use two penalised (logistic) regression models. First, a Lasso regression that uses the $L_1$-norm is performed. We will again only use the training set to fit the model. Figure 12.A clearly shows that when the penalty parameter $\lambda$ increases the estimates are shrunken towards zero. When it hits zero, it remains zero. Hence, choosing $\lambda$ is related to feature selection.
...
mCvLasso <- cv.glmnet(x = as.matrix(trainX), y = as.matrix(trainY), alpha = 1, family  = "binomial", 
...
Two vertical lines are added on figure 12.B. The first one gives the $\log(\lambda)$ for which we find the highest AUC. We will go on with this value, because with the other choice for $\lambda$, almost no genes are used anymore. We are left with 12 predictors from the original 10,000 genes (i.e. a sparse solution). The values of the corresponding $\beta$ coefficients are shown on figure 13 and next to this figure, we show the corresponding names of the genes. These genes are hence the most important in predicting the kidney transplant rejection.
