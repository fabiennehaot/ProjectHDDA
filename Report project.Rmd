...
n_PC
```
We first start with doing the PCA, where we can see we have 175 principal components that can be used. 
...
```
With the full model, we get the warning message: Algorithm did not converge. This means there is a perfect
separation and we will need penalized regression.
The full model has an area under the curve (AUC) of 0.62.

The plot shows the number of optimal PC's is 16.
