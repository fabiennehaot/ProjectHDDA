...
cex_txt <- 1; loc_txt <- 3; adj_txt <- 0.5; padj_txt <- 0

fig_nr <- 1
=======
library(ggplot2)
>>>>>>> 6933cd47ed227cafe0a25ffef01b897568c26630
```
...
grid.arrange(plot_first_view,top= paste0("Figure ",fig_nr,": Data exploration"))
fig_nr <- fig_nr + 1
X <- scale(X_raw)
```
...
grid.arrange(plot_var,top= paste0("Figure ",fig_nr,": Variance explained by the first principal components"))
fig_nr <- fig_nr + 1
...
grid.arrange(p_scatter, p_dens, ncol = 2,
             top= paste0("Figure ",fig_nr,": Principal component analysis"))
fig_nr <- fig_nr + 1

=======
  
grid.arrange(p_scatter, p_dens, ncol = 2, top = textGrob("Figure 2: Principal component analysis", 
                                                       gp = gpar(fontsize = 19)))
>>>>>>> 6933cd47ed227cafe0a25ffef01b897568c26630
```

The first 2 PCs explain 19% of the variability of X and with the first 50 we obtain 63% of the variability of X. 
...
We notice that the PCs do not clearly separate the accepted and rejected cases. However, we do see that the rejected cases are more present in the bottom left corner and the accepted cases in the top right corner. Figure `r fig_nr-1`B confirms this observation.
=======
To get more insight in the research question, we examine whether kidney rejection is associated with the PCs. For this, we look at Figure 2.A. We notice that the PCs do not clearly separate the accepted and rejected cases. However, we do see that the rejected cases are more present in the bottom left corner and the accepted cases in the top right corner. + interpretation figure 2B
>>>>>>> 6933cd47ed227cafe0a25ffef01b897568c26630
...
grid.arrange(hist_loading1, hist_loading2, ncol = 2,
             top= paste0("Figure ",fig_nr,": Histogram for PC-loadings"))
fig_nr <- fig_nr + 1
...
plot(Z[, 1], Z[, 2],
     ...
     main = "A. All genes for PC1 and PC2"
)

plot(SPC1, SPC2,
     col = cols[Y], xlab = "SPC1", ylab = "SPC2", pch = 16,
     main = paste("B.", non_zero1, "genes for SPC1 and", non_zero2, "genes for SPC2")
)
>>>>>>> 6933cd47ed227cafe0a25ffef01b897568c26630

grid.arrange(p_scatter_sparse, p_dens_sparse, ncol = 2, 
             top = paste0("Figure ",fig_nr,": Visualization of the first two sparse PCs"))
fig_nr = fig_nr + 1
```

<<<<<<< HEAD
For both PC1 and PC2 around 200 genes are important (Figure `r fig_nr-1`B), which can provide more or less the same result as the PCA with all the genes (Figure `r fig_nr-1`A).

Finally, because PCA only takes the genes into account in the decomposition and not the class membership, we executed LDA. Furthermore, because the genes are collinear, a sparse LDA is performed as well. The results of these are illustrated in Figures `r fig_nr`A and B.
=======
For both PC1 and PC2 around 200 genes are important (Figure 4B), which can provide more or less the same result as the PCA with all the genes (Figure 4.A).
>>>>>>> 6933cd47ed227cafe0a25ffef01b897568c26630

Finally, because PCA only takes the genes into account in the decomposition and not the class membership, we executed LDA. Furthermore, because the genes are collinear, a sparse LDA is performed as well. The results of these are illustrated in Figures 5.A and 5.B.
...
grid.arrange(lda_box, slda_box, ncol = 2, 
             top = paste0("Figure ",fig_nr,": Linear discriminant analysis"))
fig_nr = fig_nr + 1
...
=======
Both the regular LDA (Figure 5.A) and the sparse LDA (Figure 5.B) separate the accepted cases (red) and the rejected cases (blue) fairly well. However, the sparse LDA only needs 52 genes to do this. With the sparse LDA we see that the accepted cases are situated just below 0 and the rejected cases around 1.
...
<<<<<<< HEAD
             top = paste0("Figure ",fig_nr,": Multiple t-tests unadjusted"))
fig_nr = fig_nr + 1
=======
             top = textGrob("Figure 6: Multiple t-tests unadjusted", gp = gpar(fontsize = 19)))
>>>>>>> 6933cd47ed227cafe0a25ffef01b897568c26630
...
<<<<<<< HEAD
The standard normal curve is too narrow to fit the distribution (Figure `r fig_nr-1`A). This can happen when the genes are highly correlated or when the null hypothesis does not hold. The histogram of the p-values (Figure `r fig_nr-1`B) shows a clear peak for p-values lower than 5%, but there are too many p-values between 5% and 35%. We cannot execute the @BH:1995 procedue now, because this uses the standard normal null distribution to calculate the FDR.
=======
The standard normal curve is too narrow to fit the distribution (Figure 6.A). This can happen when the genes are highly correlated or when the null hypothesis does not hold. The histogram of the p-values (Figure 6.B) shows a clear peak for p-values lower than 5%, but there are too many p-values between 5% and 35%. We cannot execute the @BH:1995 procedure now, because this uses the standard normal null distribution to calculate the FDR.
>>>>>>> 6933cd47ed227cafe0a25ffef01b897568c26630
...
<<<<<<< HEAD
             top = paste0("Figure ",fig_nr,": Multiple t-tests: scaling and FDR-adjustment"))
fig_nr <- fig_nr + 1
=======
              top = textGrob("Figure 7: Multiple t-tests: scaling and FDR-adjustment",
                             gp = gpar(fontsize = 19)))
>>>>>>> 6933cd47ed227cafe0a25ffef01b897568c26630
...
Standardizing the z-values does not help (Figure 7.A), the theoretical distribution is now too wide to apply the @BH:1995 procedure.  
...
#local fdr
par(mfrow=c(1,1), omi = c(0, 0, 2, 0), oma = c(0, 0, 1, 0))
...
#local fdr
fdr_x <- locfdr(t_stats$z_val, plot = 4, main = "A. Histogram")  # change title other plots?
prop_h0 <- fdr_x$fp0['mlest', 'p0']
mtext("Figure 8: Local FDR", line = 1, outer = TRUE, cex = 1.5, side = 3)
>>>>>>> 6933cd47ed227cafe0a25ffef01b897568c26630
...
grid.arrange(plot_lfdr, 
             top = paste0("Figure ",fig_nr,": Multiple t-tests: Local FDR"))
fig_nr = fig_nr + 1
=======
  
grid.arrange(plot_lfdr, top = textGrob("Figure 9: Multiple t-tests: Local FDR",
                             gp = gpar(fontsize = 19)))
...
```
<<<<<<< HEAD
First we try to fit a model with all 175 PCs, using crossvalidation. Via crossvalidation will the training set be divided into 9 approximately equal subsets. Here we get an area under the curve (AUC) of `r round(full_model_cv$delta[1],2)`. However, because we use all PCs, we got a warning for overfitting the data.
=======

First we try to fit a model with all 175 PCs, using crossvalidation. Via crossvalidation will the training set be divived into 9 approximately equal subsets. Here we get an area under the curve (AUC) of `r round(full_model_cv$delta[1],2)`. However, because we use all PCs, we got a warning for overfitting the data.
>>>>>>> 6933cd47ed227cafe0a25ffef01b897568c26630
...
grid.arrange(plot_pcr_auc1, plot_pcr_auc2, ncol = 2, 
             top = paste0("Figure ",fig_nr,": Optimal number of PCs")) # plot the 2
fig_nr <- fig_nr+1
=======
  xlab("Number of PCs") +
  theme_bw() -> p2

grid.arrange(p1, p2, ncol = 2, top = textGrob("Figure 10: Optimal number of PCs",
                             gp = gpar(fontsize = 19)))
>>>>>>> 6933cd47ed227cafe0a25ffef01b897568c26630
```

In a second step, we loop and repeat this for each PC. First, we loop over the 175 PCs, in steps of 10, to decide in which region we will zoom in (Figure 10.A). Here, we get `r nr_pc_opt1` as optimal number of PCs. To get a more accurate look, we zoom in between 1 and 20 PCs and repeat the same process, but with steps of 1 (Figure 10.B). This gives us an optimal number of `r nr_pc_opt2` PCs.
...
grid.arrange(plot_dens_pcr, 
             top = paste0("Figure ",fig_nr,": PCR on ", nr_pc_opt2, " PCs")) # plot 
fig_nr <- fig_nr+1
```
...
<<<<<<< HEAD
plot(mLasso, xvar = "lambda", main = "A. Coefficients in lasso regression\n")
=======
...
<<<<<<< HEAD
plot(mCvLasso, main = "B. Choosing optimal lambda\n")

mtext(paste0("Figure ",fig_nr, ": Lasso regression"), outer = TRUE, 
             cex = cex_txt, side = loc_txt, adj = adj_txt)
fig_nr <- fig_nr +1
=======
plot(mCvLasso)

mtext("Figure 12: Lasso regression", outer = TRUE, cex = 1.5, line = 3)
...
To make a decision about $\lambda$, cross-validation is used on the training data set. We use 10-fold cross-validation because the data set is relatively small, and should therefore not take too much time. We optimize the area under the receiver characteristic curve (AUC).

Two vertical lines are added on figure `r fig_nr`B. The first one gives the $\log(\lambda)$ for which we find the highest AUC. We will go on with this value, because with the other choice for $\lambda$, almost no genes are used anymore. We are left with 12 predictors from the original 10,000 genes (i.e. a sparse solution). The values of the corresponding $\beta$ coefficients are shown on figure `r fig_nr` and next to this figure, we show the corresponding names of the genes. These genes are hence the most important in predicting the kidney transplant rejection.
...
grid.arrange(lasso_genes, 
             top = paste0("Figure ",fig_nr,": Estimated coefficients Lasso"))
fig_nr <- fig_nr + 1
=======
qplot(main = "Figure 13: Estimated coefficients", summary(coef(mLassoOpt))[-1,1],
      summary(coef(mLassoOpt))[-1,3]) + xlab("gene") + ylab("beta-hat") + 
...
mtext(paste0("Figure ",fig_nr, ": Coefficients ridge regression"), outer = TRUE, 
             cex = cex_txt, side = loc_txt, adj = adj_txt)
fig_nr <- fig_nr + 1
...
mtext("Figure 14: Coefficients ridge regression", outer = TRUE, cex = 1.5, line = 1)
...
Next, we evaluate this final model on our test set. First, the corresponding ROC curve is examined (Figure 15). Our model clearly performs better than random allocation of cases to the 2 groups.
...
<<<<<<< HEAD
grid.arrange(roc, 
             top = paste0("Figure ",fig_nr,": ROC curve"))
fig_nr <- fig_nr + 1
=======
print(roc + ggtitle("Figure 15: ROC curve") + theme(plot.title = element_text(size = 19)))
>>>>>>> 6933cd47ed227cafe0a25ffef01b897568c26630
